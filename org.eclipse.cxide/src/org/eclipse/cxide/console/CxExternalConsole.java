 ...
 *
 */
public class CxExternalConsole extends IOConsole {
	
    private static final String consoleName = "CxProlog External Console";
	CxExternalConsole consol;
	...
	ReadInitialInput readInitial;
	
	//Threads para executar os runnables acima 
	Thread readCx_thread;
    Thread readConsoleWriteCx_thread;
    Thread readInitial_thread;
    
    static BufferedWriter writeCx;
    static BufferedWriter writeConsole;
    static boolean need_main=false;
    static boolean main_time=false;
    public CxExternalConsole() {
        super(consoleName, null);
        System.out.println("D-> CxExternalConsole Starting...");
	...
	private static class ReadCxWriteConsole implements Runnable {
		...
		public void run() {
			...
			try {
				...
				System.err
						.println("Erro no CxExternalConsole.ReadCxFeedback.run()");
				e.printStackTrace();
			} catch (IOException e) {
				System.err
						.println("Erro no CxExternalConsole.ReadCxFeedback.run()");
	...
	public void restart(){
		...
		for(int i=0; i<con.length; i++){
			if(con[i].getName().equals("CxProlog External Console")){
	...
	private static class ReadInitialInput implements Runnable {
			//buff para ler do cxprolog
			BufferedReader cxBuff;
			//para escrever para a consola
			private PrintWriter outConsole;
			ReadInitialInput(InputStream cxInputStream, OutputStream outConsoleStream) {
					System.out.println("D -> Starting Initial Input filter...");
					cxBuff= new BufferedReader(new InputStreamReader(cxInputStream));
					outConsole = new PrintWriter(new OutputStreamWriter(outConsoleStream));
			}
		@Override
			...
			while(keepON){
				try {
					if(cxBuff.ready()){
							...
							//outConsole.print((char) c);
							//outConsole.flush();
							if(buff.contains("yes")){
								keepON=false;
								if(!buff.contains("main")){
									need_main=true;
									System.out.println("NEEDS MAIN");
								}
							}
}else{
	try {
	...
	public void createCxProcess(){
		try {
			...
			System.out.println("Local Instalation: "+local_instalation);
			...
			w.flush();
			Thread.sleep(100);
			readInitial= new ReadInitialInput(is, outConsoleStream);
			readConsoleWriteCx_thread = new Thread(readInitial);
			readConsoleWriteCx_thread.start();
			int c=0;
			Thread.sleep(10);
			//Ler algum lixo como por exemplo o output da mudança de directoria
			/*version = b.readLine();
			System.out.println("READ LINEBREAK:"+version);
			if(version.contains("version")){
				outConsole.print(version);
				outConsole.flush();
			}
			...
			*/
			...
			System.err.println("Erro CxExternalConsole.createCxProcess...");
			e.printStackTrace();
		}
	}
